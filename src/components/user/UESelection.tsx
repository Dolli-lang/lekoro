...
const UESelection = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      {/* --- HEADER --- */}
      <header className="border-b bg-card px-4 py-3 sticky top-0 z-10 shadow-sm">
      ...
      <section className="bg-gradient-to-b from-primary/5 to-background pt-12 pb-8 px-4 text-center">
        <div className="container mx-auto max-w-3xl">
          ...
          <p className="text-sm bg-primary/10 inline-block px-3 py-1 rounded-full text-primary font-medium">
            Plateforme dédiée aux étudiants en Mathématiques et Informatique.
          </p>
        </div>
      </section>

      {/* --- MAIN CONTENT (Le composant de sélection) --- */}
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {departements.map((dept) => (
                    <Card key={dept.id} className="cursor-pointer hover:shadow-lg transition-all hover:-translate-y-1" onClick={() => handleDepartementClick(dept)}>
                      <CardHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {ues.map((ue) => (
                    <Card key={ue.id} className="cursor-pointer hover:shadow-lg transition-all" onClick={() => handleUEClick(ue)}>
                      ...
                      <CardContent><Button variant="outline" className="w-full hover:bg-accent hover:text-white">Voir les corrigés</Button></CardContent>
                    </Card>
                  ))}
                </div>
              </div>
            ...
            {exercices.map((ex) => (
              <Button key={ex.id} variant="outline" className="justify-start h-auto py-3 px-4" onClick={() => handleExerciceSelect(ex)}>
                <div className="text-left">
                  <div className="font-bold">Exercice {ex.numero}</div>
      ...
      {/* Galerie des Corrigés (AVEC FIX TREMBLEMENT) */}
        ...
        <DialogContent className="max-w-4xl max-h-[90vh]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 overflow-y-auto p-1">
            {allImages.map((url, idx) => (
              <div 
                key={idx} 
                className="relative group cursor-pointer overflow-hidden rounded-lg shadow-sm border bg-muted"
              ...
              >
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-black/0 transition-all duration-200 group-hover:bg-black/20 pointer-events-none">
                  <span className="text-white font-medium opacity-0 group-hover:opacity-100 bg-black/40 px-3 py-1 rounded-full text-xs">
                    Page {idx + 1}
                  </span>
                </div>
              </div>
